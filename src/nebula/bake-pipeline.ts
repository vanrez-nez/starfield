...
import type {
  BakeJob,
  NebulaBakePipelineArgs,
  PatchDescriptor,
  PatchRenderTarget,
} from "./types";

export function createNebulaBakePipeline({
  renderer,
  bakeCamera,
  nebulaScene,
  nebulaUniforms,
  ...
  function renderPatchDescriptor(descriptor: PatchDescriptor, target: PatchRenderTarget): void {
    nebulaUniforms.uTileUvMin.value.copy(descriptor.storageUvMin);
    nebulaUniforms.uTileUvSize.value.copy(descriptor.storageUvSize);
    renderer.setRenderTarget(target);
    renderer.clear();
    renderer.render(nebulaScene, bakeCamera);
  }
